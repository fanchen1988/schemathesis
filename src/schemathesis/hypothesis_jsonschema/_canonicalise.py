...
def upper_bound_instances(schema):
...
def get_number_bounds(
    schema, **_3to2kwargs):
    if '_for_integer' in _3to2kwargs: _for_integer = _3to2kwargs['_for_integer']; del _3to2kwargs['_for_integer']
    else: _for_integer =  False
...
def canonicalish(schema):
    ...
    if not isinstance(schema, dict):
        raise InvalidArgument(
            "Got schema={schema} of type {schema_type}, but expected a dict.".format(schema=schema, schema_type=type(schema).__name__)
        )
...
class LocalResolver(jsonschema.RefResolver):
    def resolve_remote(self, uri):
        raise jsonschema.exceptions.RefResolutionError(
            "hypothesis-jsonschema does not fetch remote references (uri='{uri}')".format(uri=uri)
        )


def resolve_all_refs(schema, **_3to2kwargs):
    if 'resolver' in _3to2kwargs: resolver = _3to2kwargs['resolver']; del _3to2kwargs['resolver']
    else: resolver =  None
    """
    ...
    if not isinstance(resolver, jsonschema.RefResolver):
        raise InvalidArgument(
            "resolver={resolver} (type {resolver_type}) is not a RefResolver".format(resolver=resolver, resolver_type=type(resolver).__name__)
        )
    ...
    def res_one(s):
        ...
        with resolver.resolving(ref) as got:
            ...
            m = merged([s, got])
            if m is None:
                msg = "$ref:'{ref}' had incompatible base schema '{s}'".format(ref=ref, s=s)
